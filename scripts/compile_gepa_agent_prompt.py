...
def _gateway_lm(*, max_tokens: int) -> dspy.LM:
    ...
    state = json.loads((PROJECT_ROOT / "state" / "modal" / "qwen-server.json").read_text(encoding="utf-8"))
...
def main() -> int:
    ...
    parser.add_argument("--output-dir", type=Path, default=PROJECT_ROOT / "state" / "dspy" / "gepa-agent-prompt")
