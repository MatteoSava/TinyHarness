...
## Project Overview
TinyHarness benchmarks coding-agent behavior on Terminal-Bench 2.0. The v0 path runs a Claude-Code-style agent through Anthropic's Python Agent SDK, but routes inference to a Modal-hosted Qwen gateway backed by `llama.cpp` and LiteLLM.
...
- Open the local MLflow UI: `uv run tinyharness mlflow-ui`
...
- MLflow uses `sqlite:///./state/mlflow/mlflow.db` with artifacts in `./artifacts/mlflow`.
